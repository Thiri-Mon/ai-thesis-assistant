...
def load_embedding_model():
    return HuggingFaceEmbeddings(
        ...
        model_kwargs={"local_files_only": True} # False for downlaoding
    )
...
if st.sidebar.button("Store into Database"):
    if uploaded_files:
        with st.sidebar.spinner("Vectorizing data into Vector Database ..."):
            ...
            # 💡 Individual Sheet of Excel fiel reading system
                ...
                try:
                    ...
                    if uploaded_file.name.endswith(('.xlsx', '.xls')):
                        ...
                        for sheet in sheet_names:
                            ...
                            # 2. Default for  'Thesis Title' Column in Index 3 
                            if target_col_index is None:
                                target_col_index = 3 if len(df.columns) > 3 else 0
                                start_row_index = 1
                            
                            # 3. Data putting in document list
            ...
            if documents:
                #  Windows WinError passed by  DB connection in Memo
                if "vector_store" in st.session_state:
                    st.session_state.vector_store = None
                gc.collect()
                
                
                # 💡Chroma data reset without deleting folder
                try:
                    import chromadb
                    persistent_client = chromadb.PersistentClient(path="./chroma_db_storage")
                    # Old collection clear
                    for collection in persistent_client.list_collections():
                        persistent_client.delete_collection(collection.name)
                except Exception:
                    pass # Skip for the first time building
                
             
                # 💡 New Database rebuilt with  ids
                computed_ids = [f"doc_{idx}_{doc.metadata.get('row', index)}" for idx, doc in enumerate(documents)]
                
                st.session_state.vector_store = Chroma.from_documents(
                    documents=documents,
                    embedding=embeddings,
                    ids=computed_ids, # clear for data line
                    persist_directory="chroma_db_storage", # 🟢 FIXED: Removed path symbols to force a clean local folder
...
# If ChromaDB already exists, Success Box
# 💡 Sidebar footer for total data
    ...
    try:
        ...
        if 'metas' in db_data and db_data['metas']:
            ...
            for file_name, count in file_counts.items():
                summary_text += f"• {file_name}: **{count}** titles included\n"
                
            st.sidebar.info(f"📊 **Each File Storation**\n{summary_text}")
    except Exception as e:
        pass
...
# 4. Chat Engine 
# =========================================================================
# Refresh ,then previous chat history showing
...
if user_query := st.chat_input("Enter your Thesis Title for duplicate check..."):
    ...
    with st.chat_message("assistant"):
        ...
        # User greeting or thesis title check
                # =========================================================================
        # 🔴 FIXED ROUTER ENGINE: SEPARATES GREETINGS FROM RAW PROPOSALS
        # =========================================================================
        query_lower = user_query.lower()
        
        # 1. checking just greeting or making a short conversational statement
